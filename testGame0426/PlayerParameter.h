...
struct PlayerParameter
{
	...
	float SearchEnemyDistance = 450.0f;	// 敵検索距離
	...
	std::vector<AttackStep> combo =
	{
		// 1段目：出が早い（発生最速、隙も小さく牽制に最適）
		{
			...
			.attackMoveSpeed = 4.0f,
			.attackHitRadius = 16.0f,
			.attackForwardOffset = 30.0f,

			.knockBackPower = 5.0f      // 敵が離れすぎないように低め
		},
		...
		// 3段目：前進強い（大きく踏み込んで間合いを詰める、または敵を押し込む）
		{
			...
			.attackMoveSpeed = 5.0f,   // ★前進力を大幅強化（グッと踏み込む）
			.attackHitRadius = 22.0f,   // 踏み込む分、判定も少し広めに
			.attackForwardOffset = 30.0f, // 前方に判定を突き出す

			.knockBackPower = 5.0f
		},
		// 4段目：隙大きめ（5段目への大振りの溜め。威力・疲労が高いが外すと危険）
		{
			.windupTime = 25.0f,         // ★前振りを長くして「大振り感」を演出
			.activeTime = 5.0f,
		...
		{
			.windupTime = 50.0f,        // ★最大級のタメ（ヒットストップやエフェクトが映える時間）
			.activeTime = 5.0f,
			...
			.attackMoveSpeed = 7.0f,   // 最後の押し込み
			.attackHitRadius = 35.0f,   // 巻き込みを広く（超広範囲）
			.attackForwardOffset = 40.0f,

			.knockBackPower = 10.0f     // ★超強力な吹き飛ばし
		}
	};
	...
	std::vector<AttackStep> SPAttack =
	{
		...
		},
		{
			...
			.attackHitRadius = 50.0f,
			.attackForwardOffset = 30.0f,

			.knockBackPower = 15.0f
		}
	};
};